...
def compute_metrics_stats(trial, true, pred, data=None, target_metrics=None):
    ...
    # 統計量計算
    stats = {
        'means': res.mean().to_dict(),
        'stds': res.std().to_dict()
    }
    ...
    for column in df.columns:
        if '_mean' in column:
            ...
            if std_column in df.columns:
                parsed_data[base_name] = round(df[column], 4).astype(str) + ' (± ' + round(df[std_column], 4).astype(str) + ')'
    
    return pd.DataFrame(parsed_data)[['ACC', 'AUPR', 'AUROC', 'Balanced_ACC', 'F1', 'Recall', 'Precision']]
